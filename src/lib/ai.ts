...
export function buildMasterSystem(chart: BaziChart, ziwei: ZwChart | null, memories: string[] = [], returning = false): string {
  ...
  return [
    ...
    '6. 每次回答的最后必须另起一行，格式「建议：问题一｜问题二｜问题三」——由你猜命主此刻最想追问的三个【具体】问题填进去，各不超过12字，紧贴这次回答的内容，不许重复已答、不许总是那几个。举例：刚讲完事业可写「建议：几时能升职｜适合创业吗｜正偏财哪个旺」。绝对不要原样输出"问A""问B""问C"或"问题一"这类占位字样。',
...
export function buildGuaSystem(g: GuaInfo, question: string, category: string): string {
  return [
    ...
    '6. 每次回答最后另起一行，格式「建议：问题一｜问题二｜问题三」——由你猜命主此刻最想追问的三个【具体】问题填进去（各≤12字，紧贴此卦与他的问题）。举例：「建议：几时有转机｜该主动还是等｜有无小人」。绝对不要原样输出"问A""问B""问C"或"问题一"这类占位字样。',
    '',
    '=== 本次卦象（一切判断只能基于此） ===',
    `起卦方式：${g.method}`,
...
// 过滤掉模型偶尔照抄的占位符（问A/问B/问C、问题一…）——只留真问题
export function isRealSuggest(x: string): boolean {
  if (!x) return false
  return !/^问[A-Za-z]$/.test(x) && !/^问题[一二三四1-4]$/.test(x) && !/^[ABC]$/.test(x)
}
...
export function parseSuggestReply(raw: string): { body: string; suggests: string[] } {
  ...
  if (m) {
    suggests.push(...m[1].split(/[|｜、；;]/).map((x) => x.trim()).filter(isRealSuggest).slice(0, 4))
